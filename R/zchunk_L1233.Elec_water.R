...
module_water_L1233.Elec_water <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A23.CoolingSystemShares_RG3 %>%
      ...
      filter(year %in% c(HISTORICAL_YEARS, FUTURE_YEARS)) %>% # Written by Jeon (from Original JGCRI-GCAM)
    ...
    elec_tech_water_map_cal %>%
      ...
      filter(year %in% HISTORICAL_YEARS) %>% # Written by Jeon (from Original JGCRI-GCAM)

...
CONV_TWH_EJ    <- 3.6e-3
CONV_TOE_GJ    <- 41.868                # https://en.wikipedia.org/wiki/Tonne_of_oil_equivalent

CONV_TOE_KTOE  <- 1e-3
CONV_TOE_EJ    <- CONV_TOE_GJ * CONV_GJ_EJ
CONV_KTOE_EJ   <- CONV_TOE_EJ / CONV_TOE_KTOE
...
# Degree day norms
gcamusa.BASE_HDD_USA <- 4974 # https://www.eia.gov/totalenergy/data/annual/showtext.php?t=ptb0107
gcamusa.BASE_CDD_USA <- 1514 # https://www.eia.gov/totalenergy/data/annual/showtext.php?t=ptb010
...
## gcam korea
gcamkorea.RegionNum <- 28
gcamkorea.REGION <-"South Korea"
gcamkorea.STATES<- c("SU", "IC", "DJ", "DG", "GJ", "BS", "US", "GG", "GW", "CB", "CN", "JB", "JN", "GB", "GN", "JJ")

gcamkorea.DEFAULT_COEFFICIENT <- 1
gcamkorea.DEFAULT_LOGIT_TYPE  <- NA  # default logit type
gcamkorea.DEFAULT_LOGITEXP    <- -3
gcamkorea.DEFAULT_MARKET      <- gcamkorea.REGION
gcamkorea.DEFAULT_SHAREWEIGHT <- 1

gcamkorea.WIND_BASE_COST_YEAR <- 2005

gcamkorea.USE_REGIONAL_ELEC_MARKETS <- TRUE
gcamkorea.ELECT_TD_SECTORS  <- c("elect_td_bld", "elect_td_ind", "elect_td_trn")


gcamkorea.USE_REGIONAL_FUEL_MARKETS  <- TRUE
gcamkorea.REGIONAL_FUEL_MARKETS <- c("regional coal", "delivered coal", "wholesale gas", "delivered gas",
                                   "refined liquids industrial", "refined liquids enduse")

gcamkorea.STATE_RENEWABLE_RESOURCES <- c("distributed_solar", "geothermal", "onshore wind resource")
gcamkorea.STATE_UNLIMITED_RESOURCES <- c("global solar resource", "limestone")
gcamkorea.GEOTHERMAL_DEFAULT_EFFICIENCY <- 0.1

gcamkorea.GRID_REGION_LOGIT      <- -6
gcamkorea.GRID_REGION_LOGIT_TYPE <- "relative-cost-logit"

gcamkorea.DIGITS_CALOUTPUT          <- 7    # production
gcamkorea.DIGITS_DEPRESOURCE        <- 1
gcamkorea.DIGITS_COST               <- 4
gcamkorea.DIGITS_DEPRESOURCE        <- 1
gcamkorea.EFFICIENCY_PARTITION_YEAR <- 2005
gcamkorea.DIGITS_TRNUSA_DEFAULT     <- 5

gcamkorea.SECTOR_EN_NAMES <- "refining"

# Some xml delimiter
gcamkorea.STATE_SUBSECTOR_DELIMITER <- " "

gcamkorea.REGIONAL_FUEL_MARKETS <- c("regional coal", "delivered coal", "wholesale gas", "delivered gas",
                                   "refined liquids industrial", "refined liquids enduse")

# Degree day norms
# The values mean population weighted Degree of days in Korea in 2010
gcamkorea.BASE_HDD <- 4974 #4524 # https://www.eia.gov/totalenergy/data/annual/showtext.php?t=ptb0107
gcamkorea.BASE_CDD <- 1514 #1215 # https://www.eia.gov/totalenergy/data/annual/showtext.php?t=ptb010

# The internal gains are from national iternal gain
gcamkorea.INTERNAL_GAINS_SCALAR_H <- -1098
gcamkorea.INTERNAL_GAINS_SCALAR_C <- 216

gcamkorea.FERT_LOGIT_EXP  <- -3             # Define default logit expoent used in the fertlizer subsector
gcamkorea.FERT_LOGIT_TYPE <- NA
gcamkorea.FERT_NAME       <- "N fertilizer" # Define GCAM-USA category name of fertilizer


#Exchange rate
#Source: World Bank, https://data.worldbank.org/indicator/PA.NUS.FCRF?end=2017&locations=KR&start=1960&view=chart
gcamkorea.Won97ToUSD97 <- 951.29
gcamkorea.Won09ToUSD05 <- 1024.12
gcamkorea.Won09ToUSD09 <- 1276.93

gcamkorea.deflator.Won97ToWon09 <- 1.30907
